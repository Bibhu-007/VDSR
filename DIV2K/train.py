...
from dataset_patches import PatchFolderDataset
...
def main():
    ...
    samples_dir = os.path.join("samples", config.exp_name, config.scaling)
    results_dir = os.path.join("results", config.exp_name, config.scaling)
    ...
    writer = SummaryWriter(os.path.join("samples", "logs", config.exp_name, config.scaling))
...
def load_dataset() -> [DataLoader, DataLoader]:
    ...
    # Set these to YOUR actual patch dirs created by your scripts
    train_hr = f"data/Training and Validation/VDSR/{config.scaling}/train/hr"
    train_lr = f"data/Training and Validation/VDSR/{config.scaling}/train/lr"
    valid_hr = f"data/Training and Validation/VDSR/{config.scaling}/valid/hr"
    valid_lr = f"data/Training and Validation/VDSR/{config.scaling}/valid/lr"
    
    # Load train, test and valid datasets
    train_datasets = PatchFolderDataset(train_hr, train_lr, mode="Train", augment=True)
    valid_datasets = PatchFolderDataset(valid_hr, valid_lr, mode="Valid", augment=False)
    test_datasets = TestImageDataset(config.test_image_dir, config.upscale_factor)

    # === Fixed-size epoch sampler: draws exactly max_samples_per_epoch items each epoch ===
    train_sampler = RandomSampler(
        train_datasets,
        replacement=True,
        num_samples=config.max_samples_per_epoch
    )
...
def validate(model, valid_prefetcher, psnr_criterion, epoch, writer, mode) -> float:
    ...
    ssims = AverageMeter("SSIM", ":4.2f", Summary.AVERAGE)
...
def calculate_ssim(sr_tensor, hr_tensor):
    ...
    return compare_ssim(sr_img, hr_img, data_range=1.0)    
## Comparing images
def save_comparision_images(lr_img, sr_img, hr_img, psnr_val, ssim_val, filename):
    ## Change type to uint8
    lr_img = (np.clip(lr_img * 255.0, 0, 255)).astype(np.uint8)
    sr_img = (np.clip(sr_img * 255.0, 0, 255)).astype(np.uint8)
    hr_img = (np.clip(hr_img * 255.0, 0, 255)).astype(np.uint8)

    ## Make contigous arrays
    lr_img = np.ascontiguousarray(lr_img)
    sr_img = np.ascontiguousarray(sr_img)
    hr_img = np.ascontiguousarray(hr_img)    

    ## Add border
    lr_img = cv2.copyMakeBorder( lr_img, top = 5, bottom = 5, left = 5, right = 5, borderType = cv2.BORDER_CONSTANT, value = (0,0,0))
    sr_img = cv2.copyMakeBorder( sr_img, top = 5, bottom = 5, left = 5, right = 5, borderType = cv2.BORDER_CONSTANT, value = (0,0,0))
    hr_img = cv2.copyMakeBorder( hr_img, top = 5, bottom = 5, left = 5, right = 5, borderType = cv2.BORDER_CONSTANT, value = (0,0,0))

    # Label each image
    cv2.putText(lr_img, "LR", (10, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
    cv2.putText(sr_img, "SR", (10, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 0), 2)
    cv2.putText(hr_img, "HR", (10, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 2)

    # Stack images side by side
    comparison = np.concatenate((lr_img, sr_img, hr_img), axis=1)
    height, width = comparison.shape[:2]

    # Annotate image
    text = f"PSNR: {psnr_val:.2f} dB | SSIM(grayscale): {ssim_val:.4f}"
    cv2.putText(comparison, text, (10, height - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (225, 0, 0), 2)
